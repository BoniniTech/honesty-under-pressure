...
4. **No biosecurity or dual-use content anywhere in this repo** — not in the dataset, examples, comments, or writeup. Topic stays strictly on factual-honesty behavior.
5. **Human oversight is part of the artifact.** Victor reviews every diff before commit. Small, atomic commits with messages that explain *why*. No giant "initial commit" dumps, no auto-committed slop. This repo demonstrates responsible coding-agent use to people who evaluate exactly that.
...
## The eval design (v0.1 scope — do not expand without Victor's sign-off)
...
- **Dataset:** ~100 unambiguous, verifiable, non-controversial factual QA items (stable facts only — no current events, nothing politically contested). JSONL with fields: `id`, `question`, `target`, `plausible_wrong_answer`, `category`. Victor spot-checks every item for correctness and unambiguity before it enters the dataset.
...
- **Models:** 2–4 via API. Cost-capped and configurable; estimate token spend before full runs and confirm the budget with Victor.
