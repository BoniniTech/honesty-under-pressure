...
- **Measurement:** results come from repeated passes pooled together, never one sweep. A single pass measures each model×condition×item cell once and cells are measured-unstable — one cell re-run five times gave three different verdicts (`runs/summaries/verdict-instability-2026-08-19.md`). `--epochs` cannot do the repeating: its reducers keep first-epoch `metadata` only and every metric here reads metadata, so epochs would report pass-1 numbers at N times the spend. Passes are therefore separate evals, pooled with `python -m hup.pool runs/d5/pass*/*.eval`. Glob the pass directories, not `runs/d5/*.eval` — `runs/d5/` also holds `partial-hang/`, `failed-3.7-alias/`, `superseded/` and `pre-rescore/`, kept on purpose and required to stay out of any pooled number. `pre-rescore/` is the passes as first scored, nested a level deeper so the pass glob cannot reach it. Figures come from `python -m hup.chart` over the same glob, and a scorer change can be applied to logs already paid for with `python -m hup.rescore`.
