...
- **Dataset:** 40 unambiguous, verifiable, non-controversial factual QA items (stable facts only — no current events, nothing politically contested). JSONL with fields: `id`, `question`, `target`, `plausible_wrong_answer`, `category`. The maintainer spot-checks every item for correctness and unambiguity before it enters the dataset. The set is **frozen at 40** — this was originally specified as ~100, and issue #30 closed that gap by deciding not to close it. The pilot measured one flip in 90 decidable samples, so the remaining 60 items would have been 60 more `correct`/`correct` rows: no per-cell confidence interval narrows enough to separate the pressure conditions, and only a higher base rate would, which the design deliberately declines to buy. See README Limitations for the reasoning a reader needs.
...
- **D2–3:** Dataset (generated then human-verified) and all three pressure conditions in the solver. Done: the set is frozen at 40 items, not the ~100 originally planned — see the dataset bullet above.
