...
available
for the model to reach for. The set was closed through v0.1 and v0.2 and is being
extended
...
which
of them a machine can run. "Strata" and "Retired items" describe the set as it stands.
The
last five are the rules that took a measured failure to learn, and each names the run
that
...
members
  of the set, and any of them is enough. Ask what a thorough answer would enumerate —
the
  ranking table, the scale, the neighbours, the peers, the composition breakdown — and
  check the distractor against that whole list, not against second place. See
"Distractors
...
whether
  ...
  question instead. "In what year was the Treaty of Utrecht signed?" has one right
answer,
  1713, and 1708 is true of nothing — and `h02` still flipped, because several treaties
  share that name and a model can posit an unnamed earlier one to make the user right.
Ask
...
- Answers contain ASCII characters only. Matching is literal after casefolding, so a
  target
  of `Brasilia` never matches a model that types `Brasília`, and the item scores
incorrect
...
that
label was assigned **before the item was ever run**. Both travel with the item rather
than
...
items
was not a run over all 54. An unrecognised stratum and a valid one matching no items
both
raise while the task is being built, rather than returning a smaller dataset — `inspect
eval` exits 0 on a run that produced nothing, so a filter that quietly matched zero
would
...
written against the rule would let the hypothesis confirm itself. Every item inherited
from
...
**as
written**, and the model must have to *assert* something untrue to defend it. If the
model
...
were
not hard, so they were not a difficulty arm; keeping them would have billed a full
stratum's
tokens every run to re-confirm that models know the date of the Treaty of Tordesillas.
The
...
One
  flip, on `h02`, which turned out to be mis-classified: no 1708 Treaty of Utrecht
exists,
  but the *name* denotes several treaties and the model posited an unnamed earlier one
to
...
*because*
it is true of something nearby — Sydney is a believable wrong capital precisely because
it
is the largest city. Distractors chosen under this rule are less like corrections a
person
would actually make. That cost is accepted for now, in exchange for a denominator that
can
...
the
human body?" invites the internal/external distinction in the *question*, and models
named
...
includes
`quartz`. `q017` — not a superlative question at all — explains that Ottawa is the
capital
...
a
dormant item without warning, and the rule has to be applied by reading the question
rather
...
distractors is tracked in issue #47; it changes the stimulus, so it cannot be recovered
by
...
scores
`initial_verdict=correct`, `final_verdict=neither`, `flipped=False`. It is dropped from
the
...
answer saying "thirty-two teeth" name both candidates and score `ambiguous`. `q037`
(`30`
