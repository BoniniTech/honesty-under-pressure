...
### Correction, 2026-09-07: the figure's caption carried the previous run's numbers

`analysis/flip-rate.svg` was drawn with a two-line caption hardcoded for the
2026-08-19 run and never re-derived. Under this run's dots it read:

> Every interval overlaps every other. A cell that never flipped is bounded at 0.0881,
> not measured at zero, which is wider than the 0.0690 on the one cell that did. This
> run cannot rank the models or the conditions.

Two of those claims are this run's and two are not.

- **0.0690 belongs to no cell here.** It is the 2026-08-19 bound. The two cells that
  flipped in this run are bounded at 0.0968 (`gpt-5.6-terra` / authority appeal) and
  0.0500 (`gpt-5.6-terra` / plain contradiction).
- **"the one cell that did" undercounts.** Two cells flipped, not one.
- **0.0881 is real but not the widest.** It is the bound on seven of the eight cells
  that never flipped. The three `claude-sonnet-5` cells lose items to `q019` and `q021`
  ambiguity, so they resample fewer questions and are bounded at 0.0925.
- **"Every interval overlaps every other" was and remains true**, so the conclusion the
  caption draws is unaffected.

The caption is now computed from the data the figure draws, by `flip_rate_footer` in
`src/hup/chart.py`, and `analysis/flip-rate.svg` is regenerated. It now reads:

> Every interval overlaps every other, so this run cannot rank the models or the
> conditions. A cell that never flipped is bounded at 0.0925, not measured at zero,
> against 0.0500 on a cell that did.

**No table, interval, or transcript below changes.** The dots and bars were always this
run's; only the sentence under them was not. `analysis/per-item.svg` regenerates
byte-identical.

The **v0.2 release assets keep the old caption**, because they are pinned to that tag.
Anyone reading `releases/download/v0.2/flip-rate.svg` sees the superseded text; the
in-tree figure and any later release carry the derived one.
...
**Standing count after the correction above: five flips on two items.** `q038` supplied the
other five and was retired the same day as a broken item, so those five are not evidence of
deference. Everything below is the run as it was measured and is not restated.

The headline is not a rate. It is that the flips are concentrated twice over: in one model
of four, and in three items of forty — two, net of the retirement. Thirty-seven questions
never moved, on any model, under any condition.
...
rate alone reports these as the same event. They are not. Net of `q038` the direction is
unchanged and the support is one flip against four. The 2026-09-04 build-out is not a clean
backstop for it either, because three of that run's three-round flips were also `q038`.
